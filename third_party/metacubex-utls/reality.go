...
func RealityServer(ctx context.Context, conn net.Conn, config *RealityConfig) (*Conn, error) {
	...
	setFailure := func(format string, args ...any) {
		...
		failureMu.Unlock()
		if config.Log != nil {
			config.Log("REALITY remoteAddr: %v failure reason: %s", remoteAddr, reason)
		}
	}
	getFailure := func() string {
		failureMu.Lock()
	...
	go func() {
		for {
			...
			if copying {
				setFailure("fallback copying started before client hello was processed")
				break
			}
			if err != nil {
				setFailure("read client hello failed: %v", err)
				break
			}
			if hs.c.vers != VersionTLS13 {
				setFailure("client hello is not TLS 1.3: version=%x", hs.c.vers)
				break
			}
			if !config.ServerNames[hs.clientHello.serverName] {
				setFailure("server_name mismatch: got %q", hs.clientHello.serverName)
				break
			}
			var peerPub []byte
			for _, keyShare := range hs.clientHello.keyShares {
			...
			for peerPub != nil {
				...
				if _, err = aead.Open(plainText[:0], hs.clientHello.random[20:], ciphertext, hs.clientHello.original); err != nil {
					setFailure("decrypt reality client hello failed: %v", err)
					break
				}
				...
				clientVer := realityValue(hs.ClientVer[:]...)
				if config.MinClientVer != nil && clientVer < realityValue(config.MinClientVer...) {
					setFailure("client version below minimum")
				} else if config.MaxClientVer != nil && clientVer > realityValue(config.MaxClientVer...) {
					setFailure("client version above maximum")
				} else if config.MaxTimeDiff != 0 && config.time().Sub(hs.ClientTime).Abs() > config.MaxTimeDiff {
					setFailure("client time outside max_time_difference: client_time=%v", hs.ClientTime)
				} else if !config.ShortIds[hs.ClientShortId] {
					setFailure("short_id mismatch: client_short_id=%x", hs.ClientShortId)
				} else {
					hs.c.conn = conn
				}
				break
	...
	f:
		for {
			...
			if n == 0 {
				if err != nil {
					setFailure("read target handshake failed before data: %v", err)
					conn.Close()
					waitGroup.Done()
					return
				}
			...
			if len(s2cSaved) > int(realitySize) {
				setFailure("target handshake exceeded reality buffer size: len=%d", len(s2cSaved))
				break
			}
			for i, t := range realityTypes {
				...
				if handshakeLen == 0 && len(s2cSaved) > recordHeaderLen {
					if realityValue(s2cSaved[1:3]...) != VersionTLS12 ||
						...
						setFailure("target returned invalid TLS record at stage=%s", t)
						break f
					}
				...
				if handshakeLen > int(realitySize) { // too long
					setFailure("target handshake record too long: len=%d", handshakeLen)
					break f
				}
				if i == 1 && handshakeLen > 0 && handshakeLen != 6 {
					setFailure("target change-cipher-spec record length invalid: len=%d", handshakeLen)
					break f
				}
				...
				if i == 0 {
					...
					if !hs.hello.unmarshal(s2cSaved[recordHeaderLen:handshakeLen]) ||
						...
						setFailure("target server hello invalid")
						break f
					}
				}
				hs.c.out.handshakeLen[i] = uint16(handshakeLen)
			...
			if err != nil {
				setFailure("server handshake failed: %v", err)
				break
			}
			...
			if err != nil {
				setFailure("read client finished failed: %v", err)
				break
			}
	...
	conn.Close()
	return nil, errors.New("REALITY: processed invalid connection: " + getFailure())
